...
export function isVideo_metadata(text) {
	return text.includes('panonood') && !text.includes('Nai-stream');
}

// div#metadata-line
export function isScheduled_metadata(text) {
	return text.includes('Naka-iskedyul') || text.includes('Magsisimula');
}

// span#text.ytd-thumbnail-overlay-time-status-renderer[aria-label]
export function isLive_status_label(text) {
	return text === 'LIVE' || text === 'Premiere';
}

// span#text.ytd-thumbnail-overlay-time-status-renderer[aria-label]
export function isVideo_status_label(text) {
	return text.endsWith('segundo') || text.endsWith('minuto') || text.endsWith('oras');
}
...
export function isNotificationOff_button(text) {
	return text === 'Abisuhan ako' || text === 'I-notify ako';
}
