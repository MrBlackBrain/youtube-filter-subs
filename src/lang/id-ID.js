...
export function isVideo_metadata(text) {
	return text.includes('ditonton') && !text.includes('Streaming');
}

// div#metadata-line
export function isScheduled_metadata(text) {
	return text.includes('Tayang') || text.includes('perdana');
}
...
export function isVideo_status_label(text) {
	return text.endsWith('detik') || text.endsWith('menit') || text.endsWith('jam');
}
