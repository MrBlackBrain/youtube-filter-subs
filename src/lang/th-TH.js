...
export function isVideo_metadata(text) {
	return text.includes('การดู') && !text.includes('สตรีมแล้วเมื่อ');
}

// div#metadata-line
export function isScheduled_metadata(text) {
	return text.includes('กำหนดเผยแพร่') || text.includes('แสดงครั้งแรกเมื่อวันที่');
}

// span#text.ytd-thumbnail-overlay-time-status-renderer[aria-label]
export function isLive_status_label(text) {
	return text === 'สด' || text === 'วิดีโอพรีเมียร์';
}

// span#text.ytd-thumbnail-overlay-time-status-renderer[aria-label]
export function isVideo_status_label(text) {
	return text.endsWith('วินาที') || text.endsWith('นาที') || text.endsWith('ชั่วโมง');
}
