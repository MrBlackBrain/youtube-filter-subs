...
export function createRow(
	...
	switch (input_class) {
		case "step":
			div.appendChild(
				createInputArea(setting, deafult_value, onChange, undefined, input_class, minRate, maxRate, stepRate, limitRate)
			);
			break;
		default:
...
export function createRowKeyword(
	label,
	mode,
	setting,
	deafult_value,
...
function createDraggableIcon(draggable) {
	...
	if (draggable) {
		...
		div.innerHTML =
			'<svg width="14" height="24" viewBox="0 0 24 24" style="width: 24px; height: 24px;"><path d="M21 10H3V9h18v1Zm0 4H3v1h18v-1Z"></path></svg>';
	} else {
...
function createToggle(mode, setting, deafult_value) {
	...
	div.innerHTML =
		'<input id="' +
		mode +
		'" class="checkbox visibility_checkbox" type="checkbox" ' +
		(setting === true ? "checked" : setting === undefined && deafult_value ? "checked" : "") +
		' default="' +
		deafult_value +
		'" /><label for="' +
		mode +
...
function createDefaultToggle(mode, setting, tab_group) {
	...
	div.innerHTML =
		'<input id="default_' +
		mode +
		'" class="checkbox default_checkbox ' +
		(tab_group ? tab_group : "") +
		'" type="checkbox" ' +
		(setting === true ? "checked" : "") +
		' default="false" /><label for="default_' +
		mode +
		'" class="switch" />';
	return div;
}
...
export function registerResetButton(reset_button, progress_div, progress_class, done_class, default_order, progress) {
	reset_button.addEventListener("mousedown", () =>
		progress.startProgress(progress_div, progress_class, done_class, state)
	);
	reset_button.addEventListener("touchstart", () =>
		progress.startProgress(progress_div, progress_class, done_class, state)
	);
	...
	reset_button.addEventListener("touchcancel", () =>
		progress.endProgress(progress_div, progress_class, done_class, state)
	);
